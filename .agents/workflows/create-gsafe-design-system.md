...
### Bước 5: Kiểm thử (QA & Validation)

- **Hành động:** Đóng vai trò làm QA, tiến hành kiểm tra kỹ lưỡng các trang UI mới/cập nhật qua Browser.
- **Tiêu chí:**
  - Kiểm tra độ tương phản (contrast) ở cả Light/Dark mode.
  - Xác minh sự hiện diện và tương tác của các element (D-n-D regions, Boards, Badges).
  - Kiểm tra các Universal ID (Beads IDs) hiển thị đúng trên UI.
  - Tạo các Task/Issue nếu phát hiện lỗi UI/UX chưa đạt quy chuẩn.
- **Output:** Có một bản nhận xét/chỉnh sửa nếu chưa mượt mà.

### Bước 6: Phê duyệt từ Con người (Human Approval & Git Committing)
