...
**Agent Comprehension Directive:** You are operating at Layer 3 (The Details). You must navigate the Layer 2 ASCII Map above to determine *which exact* `.md` rule file to read. Do not read all rules at once. You are strictly evaluating artifacts generated by the corresponding workflow against the mechanics defined by the Root Methodology (Layer 1).
