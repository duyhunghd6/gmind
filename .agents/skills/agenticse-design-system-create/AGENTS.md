...
<!-- beads-id: br-design-create-agents -->
...
9. **Source-First Context**: `ui-contract.md` is the LLM-facing source. Compiled JSON is machine evidence for tools/auditors and may enter prompts only as small extracted YAML/TOON slices.
10. **Artifact Budget**: Human/LLM review units should stay around 1,000–2,000 lines; large storyboards, layout rules, component maps, preview manifests, and registry data must be consumed through `artifact-index.json`, `context-slices/`, review HTML, or lookup scripts.
...
- **W2 (Create & Build)**: First, read `ui-contract.md`, `flow.md`, focused review diagrams, contract summaries, and assigned `context-slices/**/*.yaml`. Query `component-map.json`, `storyboards.json`, `layout-rules.json`, and preview manifests only through `artifact-index.json`, lookup scripts, or targeted slices. Then, code baseline (v000) prototypes, rigidly mapping contract `ds_id` entries to physical `data-ds-id` DOM elements. Register their first versions in the Element History and publish to the Design System Showcase.
