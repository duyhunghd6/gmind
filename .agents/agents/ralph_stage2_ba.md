...
5. For Gate B summaries, reference these Stage 1 review inputs:
   ...
   - targeted `context-slices/` rows named by auditor/QA failures
   Treat `component-map.json`, `storyboards.json`, `layout-rules.json`, and preview manifests as machine evidence; list them for traceability but do not require raw human review.
...
- Machine evidence, listed but not required for raw human review:
  - `component-map.json`
  - `storyboards.json`
  - `layout-rules.json`
...
```json
{
  ...
  "artifacts_for_review": ["page.tsx", "ui-contract.md", "context-slices/summary/contract-summary.yaml", "flow.md", "storyboards-review.html", "preview/index.html", "artifact-index.json", "browser-render artifacts"],
  "machine_evidence": ["component-map.json", "storyboards.json", "layout-rules.json", "preview/preview-manifest.json"]
}
```
