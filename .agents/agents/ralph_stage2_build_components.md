...
- `{contract_path}/context-slices/storyboards/*.yaml` only for assigned trajectories/actions
...
Do not load full `component-map.json` or `storyboards.json` into prompt context. Treat them as compiled machine artifacts; use `artifact-index.json`, existing context slices, or grep/Python to extract only the `ds_id`, screen, state, or trajectory rows you are currently implementing, then restate those rows as compact YAML/TOON notes. Only read full JSON when it is below the reviewability threshold and the orchestrator explicitly allows it.
