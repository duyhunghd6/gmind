...
- **`gmind search <query>`:**
  ...
  - Trả kết hợp giữa Text Docs và AST Code snippet.
- **`gmind context <beads-id> [--depth N]`:**
  - Truy vấn hạt nhân. Gom toàn bộ Description (từ beads_rust/FrankenSQLite), Mã nguồn rễ (Từ Graph RAG), và Discussion History (Từ Zvec) của 1 ID cụ thể.
...
- **Execution:** Bắt buộc gọi `gmind context <id> --depth N` trước khi sửa code. Agent tùy chọn mức ngữ cảnh cần thiết.
...
- **Completion:** Commit với tag `#br-123` và chạy `br close <id>`.
