...
- **`gmind trace <beads-id> [--reverse]`:** _(Đề xuất — 2026-03-01)_
  ...
  - > ✅ **Thêm mới (2026-03-01):** Xem [spike-beads-id-in-docs.md](../researches/spikes/spike-beads-id-in-docs.md).
...
- **Traceability Tagging (bắt buộc):** Khi tạo task mới (`bd create`), Agent **PHẢI** gắn traceability tags để liên kết với Plan và PRD:
  ...
  ```
  Nếu không gắn tag, `gmind coverage` sẽ báo gap. Xem PRD-02 §3.2.
...
> 7. ~~**Không có công cụ truy vết Requirements:**~~ → **Đã thêm 4 gmind commands** (2026-03-01): `gmind trace`, `gmind coverage`, `gmind impact`, `gmind gaps`. Agent bắt buộc gắn `--tag="implements:"` và `--tag="satisfies:"` khi tạo task. Xem [spike-beads-id-in-docs.md](../researches/spikes/spike-beads-id-in-docs.md).
