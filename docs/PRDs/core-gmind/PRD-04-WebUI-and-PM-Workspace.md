...
- **AC15 (Showcase Route Coverage):** PRD-04 phải liệt kê đầy đủ các URL showcase: `/terminal`, `/portfolio`, `/pi-planning`, `/git-graph`, `/board`, `/knowledge-graph`, `/approval`, `/timeline`, `/components`, `/docs`, `/search`, `/trace/:id`, `/storyboards`, và canonical PM Space route `/webui-pm-workspace`. Header menu **PM Space** phải đứng ngay bên phải **Design System** và không quảng bá `/design-system/webui-pm-workspace` làm canonical.
...
- **AC18 (Storyboard E2E Alignment):** Storyboard overview và `/storyboards/:id` phải map use-case journeys sang screen paths, state names, expected outcomes, and E2E investigation guidance; mỗi CTA phải mở đúng screen thực tế.
