...
**3. Xây dựng công cụ `gmem` CLI**
Bạn có thể code `gmem` (có thể bằng Python để dễ dùng thư viện Zvec `[3]`) đóng vai trò là "cổng thông tin" (API gateway) cho Claude:

- `gmem search <query>`: Quét qua Zvec để tìm docs, HTML mockups hoặc AST context.
- `gmem context <beads-id>`: Tự động gom nhóm toàn bộ context bao gồm mô tả task (từ Dolt DB), mã nguồn liên quan (từ Graph RAG), và lịch sử thảo luận (từ Zvec/mcp_agent_mail).
...
- **Execution:** Trước khi bắt tay vào code, tác nhân phải gọi `gmem context <id>` để nạp bối cảnh hệ thống, tài liệu thiết kế và lịch sử của issue.
...
Cách tiếp cận này giúp bạn phân tách rõ ràng lớp dữ liệu (Dolt/Zvec), lớp giao diện (Hệ quản trị Web), và lớp công cụ máy móc (Beads CLI, `gmem`, `mcp_agent_mail`), tạo thành một hệ thống bộ nhớ vững chắc cho đội ngũ kỹ sư AI của bạn.
