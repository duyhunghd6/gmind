...
import { beadsNodes, beadsEdges, NODE_TYPE_META, EDGE_TYPE_META } from "@/data/beads-traversal-data";
...
export function BeadsTraversalShowcase({
  state = "default",
  onAction,
}: {
  state?: string;
    ...
    <div aria-label="Beads Traversal Screen">
        ...
        <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 300px" : "1fr", gap: "16px" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "6px", marginBottom: "16px", alignItems: "center" }}>
              <button
                className={`state-toggle-bar__btn${direction === "forward" ? " state-toggle-bar__btn--active" : ""}`}
                onClick={() => {
                  setDirection("forward");
                  if (onAction) onAction("EVENT_HASH_NAVIGATE", "forward");
                }}
              ...
              <button
                className={`state-toggle-bar__btn${direction === "reverse" ? " state-toggle-bar__btn--active" : ""}`}
                onClick={() => {
                  setDirection("reverse");
                  if (onAction) onAction("EVENT_HASH_NAVIGATE", "reverse");
                }}
                style={{ fontSize: "0.75rem" }}
              >
                ↑ Reverse (Commit → PRD)
              </button>
              <DsIdBadge id="ds:beadsTraversal:directionToggle-001" />
            </div>
                      ...
                      return (
                        ...
                        >
                          <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
                            <span>{NODE_TYPE_META[node.type as keyof typeof NODE_TYPE_META]?.icon}</span>
            ...
            <div style={{ display: "flex", gap: "16px", fontSize: "0.7rem", color: "var(--text-dim)", marginTop: "12px" }}>
              {layers.map((l) => (
                <span key={l.type}>
                  {NODE_TYPE_META[l.type as keyof typeof NODE_TYPE_META]?.icon} {beadsNodes.filter((n) => n.type === l.type).length} {l.label}
                </span>
              ))}
          ...
          {selected && (
            <div className="ve-card" style={{ padding: "16px", alignSelf: "start" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <span style={{ fontSize: "1.25rem" }}>{NODE_TYPE_META[selected.type as keyof typeof NODE_TYPE_META]?.icon}</span>
                <button onClick={() => setSelectedId(null)} style={{ background: "none", border: "none", color: "var(--text-dim)", cursor: "pointer" }}>✕</button>
              </div>
              <h3 style={{ fontSize: "0.9375rem", fontWeight: 600, color: "var(--text)", marginBottom: "4px", margin: 0 }}>{selected.label}</h3>
              <div style={{ fontSize: "0.7rem", fontFamily: "var(--font-mono)", color: "var(--accent-cyan)", marginBottom: "8px" }}>{selected.id}</div>
              <p style={{ fontSize: "0.8125rem", color: "var(--text-dim)", lineHeight: 1.5, marginBottom: "12px", margin: 0 }}>{selected.description}</p>
              ...
              {parents.length > 0 && (
                <div style={{ marginBottom: "12px" }}>
                  ...
                  {parents.map((pid) => {
                    ...
                    return pn ? <button key={pid} onClick={() => setSelectedId(pid)} style={{ display: "block", fontSize: "0.7rem", color: "var(--accent-cyan)", background: "none", border: "none", cursor: "pointer", padding: "2px 0", textAlign: "left" }}>{NODE_TYPE_META[pn.type as keyof typeof NODE_TYPE_META]?.icon} {pn.label} ({pn.id})</button> : null;
                  })}
                </div>
              )}
              {children.length > 0 && (
                <div style={{ marginBottom: "12px" }}>
                  ...
                  {children.map((cid) => {
                    ...
                    return cn ? <button key={cid} onClick={() => setSelectedId(cid)} style={{ display: "block", fontSize: "0.7rem", color: "var(--accent-cyan)", background: "none", border: "none", cursor: "pointer", padding: "2px 0", textAlign: "left" }}>{NODE_TYPE_META[cn.type as keyof typeof NODE_TYPE_META]?.icon} {cn.label} ({cn.id})</button> : null;
                  })}
                </div>
              )}

              <div style={{ display: "flex", flexDirection: "column", gap: "6px", borderTop: "1px solid var(--border)", paddingTop: "12px" }}>
                <button onClick={() => onAction && onAction("EVENT_VIEW_TRACE", selected.id)} style={{ background: "none", border: "none", padding: 0, textAlign: "left", fontSize: "0.75rem", color: "var(--accent-cyan)", cursor: "pointer" }}>🧠 View in Knowledge Graph →</button>
                <button onClick={() => onAction && onAction("EVENT_SEARCH", selected.id)} style={{ background: "none", border: "none", padding: 0, textAlign: "left", fontSize: "0.75rem", color: "var(--accent-cyan)", cursor: "pointer" }}>🔍 Search in Explorer →</button>
              </div>
            </div>
          )}
        </div>
      ...
      {state === "loading" && (
        <div className="ve-card" style={{ padding: "16px" }}>
          ...
          <div style={{ marginTop: "16px" }}><Skeleton count={8} /></div>
        </div>
      )}
      ...
      {state === "empty" && (
        <EmptyState
          icon="🔗"
          title="Chưa có liên kết Beads"
          desc="Gắn satisfies/implements tags khi tạo task: bd create --tag='implements:br-plan-01'"
        />
      )}
      ...
      {state === "error" && (
        <ErrorBanner
          title="Lỗi truy vết"
          message="gmind trace failed: beads_rust database locked. Retry after lock release."
      ...
      {state === "offline" && (
        <div className="offline-banner">
          <span className="offline-banner__icon">📡</span>Hiển thị cache — một số nodes có thể outdated
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <div className="forbidden-gate__desc">Beads Traversal yêu cầu vai trò Architect hoặc PMO.</div>
