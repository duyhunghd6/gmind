...
export function ExplorerShowcase({
  ...
  return (
    <div aria-label="Gmind Explorer Screen" className="space-y-4">
      {state === "default" && (
        <div>
          {/* Search Bar */}
            ...
            <div style={{ flex: 1, position: "relative" }}>
              <input
                ...
                onChange={(e) => {
                  ...
                  if (onAction) onAction("EVENT_SEARCH", e.target.value);
                }}
                style={{
          ...
          <div style={{ display: "flex", gap: "6px", marginBottom: "16px", flexWrap: "wrap", alignItems: "center" }}>
            {FILTER_TYPES.map((f) => (
              <button
                ...
                onClick={() => {
                  setFilter(f.value);
                  if (onAction) onAction("EVENT_HASH_NAVIGATE", f.value);
                }}
            ...
            <DsIdBadge id="ds:explorer:typeFilters-001" />
          </div>
            ...
            {selected && (
              <div className="ve-card" style={{ padding: "16px", alignSelf: "start" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  <button onClick={() => onAction && onAction("EVENT_VIEW_TRACE", selected.beadsId)} style={{ background: "none", border: "none", padding: 0, textAlign: "left", fontSize: "0.75rem", color: "var(--accent-cyan)", cursor: "pointer" }}>🧠 View in Knowledge Graph →</button>
                  {selected.beadsId && <button onClick={() => onAction && onAction("EVENT_VIEW_TRACE", selected.beadsId)} style={{ background: "none", border: "none", padding: 0, textAlign: "left", fontSize: "0.75rem", color: "var(--accent-cyan)", cursor: "pointer" }}>🔗 Trace Beads ID →</button>}
                  {(selected.type === "doc" || selected.type === "adr" || selected.type === "spike") && <button onClick={() => onAction && onAction("EVENT_VIEW_DOC", selected.id)} style={{ background: "none", border: "none", padding: 0, textAlign: "left", fontSize: "0.75rem", color: "var(--accent-cyan)", cursor: "pointer" }}>📄 Open in Doc Viewer →</button>}
                </div>
              </div>
            )}
          </div>
      ...
      {state === "loading" && (
        <div className="ve-card" style={{ padding: "16px" }}>
          ...
          <div style={{ marginTop: "16px" }}><Skeleton count={6} /></div>
        </div>
      )}

      {state === "empty" && (
        <EmptyState icon="🔍" title="Không tìm thấy kết quả" desc="Thử tìm kiếm với từ khóa khác hoặc mở rộng bộ lọc." />
      )}

      {state === "error" && (
        <ErrorBanner
          title="Lỗi tìm kiếm"
          message="Zvec query timeout after 5000ms. Index may need rebuilding: gmind reindex"
      ...
      {state === "offline" && (
        <div className="offline-banner">
          <span className="offline-banner__icon">📡</span>Tìm kiếm offline — chỉ hiển thị cache cục bộ
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <div className="forbidden-gate__title">Cần quyền truy cập</div>
          <div className="forbidden-gate__desc">Explorer yêu cầu quyền Observer trở lên.</div>
