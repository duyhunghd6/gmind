...
export function KanbanShowcase({
  ...
  const handleDragEnd = useCallback((result: DropResult) => {
    ...
    if (onAction) onAction("EVENT_MOVE_CARD", `moved ${result.draggableId}`);
  }, [onAction]);
  ...
  return (
    <div aria-label="Kanban Board Screen">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            {kanbanBoards.map((b) => (
              <button
                ...
                onClick={() => {
                  switchBoard(b.id);
                  if (onAction) onAction("EVENT_HASH_NAVIGATE", b.id);
                }}
        ...
        <div style={{ display: "flex", gap: "16px" }}>
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} style={{ flex: 1, minWidth: 160 }}>
              ...
              <div style={{ marginTop: 8 }}><Skeleton count={2} /></div>
            </div>
          ))}
        </div>
      )}
      ...
      {state === "empty" && (
        <EmptyState icon="📋" title="Chưa có board nào" desc="Tạo board Kanban đầu tiên để bắt đầu quản lý dự án.">
          <button className="btn-primary">+ Tạo board</button>
        </EmptyState>
      )}

      {state === "error" && (
        <ErrorBanner
          title="Không thể tải board"
          message="Lỗi đọc dữ liệu từ FrankenSQLite. Database file may be corrupted."
      ...
      {state === "offline" && (
        <div className="offline-banner">
          <span className="offline-banner__icon">📡</span>Sync bị gián đoạn — thay đổi sẽ lưu local
          <span className="offline-banner__timer">Retry in 5s</span>
        </div>
      )}
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <div className="forbidden-gate__title">Yêu cầu quyền PM</div>
          <div className="forbidden-gate__desc">Bạn cần vai trò Project Manager để truy cập Kanban Board.</div>
          <div className="forbidden-gate__code">HTTP 403 — role:pm required</div>
        </div>
      )}
    </div>
  );
