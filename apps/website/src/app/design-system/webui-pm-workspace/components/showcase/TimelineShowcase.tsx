...
export function TimelineShowcase({
  ...
  return (
    <div aria-label="Timeline Screen" className="space-y-6">
      {state === "default" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          {/* File Lease indicators */}
          <div>
            <div id="file-lease" style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
              <h3 style={{ color: "var(--text)", fontSize: "0.875rem", fontWeight: 600, margin: 0 }}>Chỉ báo Khóa tệp</h3>
          ...
          <div>
            <div id="activity-feed" style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
              <h3 style={{ color: "var(--text)", fontSize: "0.875rem", fontWeight: 600, margin: 0 }}>Nhật ký Hoạt động ({activityEvents.length} events)</h3>
          ...
          <div>
            <div id="sprint-day" style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
              <h3 style={{ color: "var(--text)", fontSize: "0.875rem", fontWeight: 600, margin: 0 }}>Timeline — Sprint Day ({timelineEntries.length} entries)</h3>
      ...
      {state === "loading" && (
        <div className="ve-card" style={{ padding: "16px" }}>
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="skeleton-row" style={{ marginBottom: "16px" }}>
      ...
      {state === "empty" && (
        <EmptyState
          ...
          desc="Nhật ký sẽ hiển thị khi các Agent bắt đầu làm việc."
        />
      )}
      ...
      {state === "offline" && (
        <div className="offline-banner">
          <span className="offline-banner__icon">📡</span>Event stream mất kết nối
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <div className="forbidden-gate__desc">Cần quyền Observer trở lên để xem Activity Feed.</div>
