...
export function TerminalShowcase({ state = "default", onAction }: { state?: string; onAction?: (e: string, target?: string) => void }) {
  const [scenarioId, setScenarioId] = useState(scenarios[0].id);

  const scenario = scenarios.find((s) => s.id === scenarioId) || scenarios[0];

  return (
    <div aria-label="Terminal Screen" className="space-y-4">
      {state === "default" && (
      ...
      {state === "empty" && (
        <EmptyState icon="💻" title="Chưa có phiên terminal" desc="Bắt đầu phiên terminal mới để xem output từ các Agent.">
          <button className="btn-primary" onClick={() => onAction && onAction("EVENT_REFRESH")}>+ Phiên mới</button>
        </EmptyState>
      )}

      {state === "error" && (
        <ErrorBanner title="Không thể kết nối terminal" message="WebSocket connection refused. Kiểm tra terminal server đang chạy." onRetry={() => onAction && onAction("EVENT_REFRESH")} fullpage />
      )}
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <button className="btn-primary" onClick={() => onAction && onAction("EVENT_BACK")}>Yêu cầu quyền</button>
