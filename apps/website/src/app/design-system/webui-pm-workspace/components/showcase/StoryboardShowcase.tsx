...
import { usecases, USECASE_MAP, type Usecase } from "@/data/usecase-data";
...
export function StoryboardShowcase({
  ...
  // Detail View Mode
  if (storyboardId) {
    const uc = USECASE_MAP[storyboardId];
    if (!uc) {
      return (
        <ErrorBanner
          title="Usecase không tồn tại"
          message={`Không tìm thấy usecase với ID: ${storyboardId}`}
          onRetry={() => onAction && onAction("EVENT_BACK")}
          fullpage
        />
      );
    }
    ...
    return (
      <div aria-label="Storyboard Detail Screen" className="space-y-6">
        {state === "default" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div style={{ display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
              <span style={{
                ...
                padding: "3px 10px",
                borderRadius: "12px",
                background: `${uc.journeyColor}18`,
                color: uc.journeyColor,
            ...
            {/* Summary */}
            <p style={{ color: "var(--text-dim)", fontSize: "0.875rem", lineHeight: 1.6, margin: 0 }}>
              {uc.summary}
            </p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
              {uc.steps.map((step, i) => (
                <div key={i} style={{ display: "flex", gap: "16px", position: "relative" }}>
                  ...
                  <div className="ve-card" style={{ flex: 1, padding: "14px 18px", marginBottom: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                      <button
                        onClick={() => onAction && onAction("EVENT_HASH_NAVIGATE", step.screen)}
                        style={{
                          cursor: "pointer",
                          fontSize: "0.7rem",
                          padding: "2px 8px",
                      ...
                      {step.state && (
                        <span style={{ fontSize: "0.65rem", padding: "1px 6px", borderRadius: "3px", background: "rgba(139,148,158,0.1)", color: "var(--text-dim)" }}>
                          {step.state}
                        </span>
                      )}
                    </div>
            ...
            {uc.relatedUsecases.length > 0 && (
              <div>
                <h3 style={{ fontSize: "0.875rem", fontWeight: 600, color: "var(--text)", marginBottom: "8px", margin: 0 }}>
                  🔗 Usecase liên quan
                </h3>
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  {uc.relatedUsecases.map((relId) => {
                    ...
                    return (
                      <button
                        key={relId}
                        onClick={() => onAction && onAction("EVENT_HASH_NAVIGATE", `/design-system/storyboard/${relId}`)}
                        className="ve-card"
                        style={{
            ...
            <div>
              <button
                onClick={() => onAction && onAction("EVENT_BACK")}
                style={{ background: "none", border: "none", fontSize: "0.8125rem", color: "var(--accent-cyan)", cursor: "pointer", padding: 0 }}
              >
        ...
        {state === "loading" && <div className="ve-card" style={{ padding: "24px" }}><Skeleton variant="card" /><div style={{ marginTop: "16px" }}><Skeleton count={4} /></div></div>}
        {state === "error" && <ErrorBanner title="Lỗi tải chi tiết" message="Usecase details failed to load." onRetry={() => onAction && onAction("EVENT_REFRESH")} fullpage />}
        {state === "offline" && <div className="offline-banner"><span className="offline-banner__icon">📡</span>Sync offline<span className="offline-banner__timer">3s</span></div>}
        {state === "forbidden" && <div className="forbidden-gate"><div className="forbidden-gate__title">Access Denied</div><div className="forbidden-gate__code">403</div></div>}
      </div>
    );
  }
  ...
  return (
    <div aria-label="Storyboard Screen">
                              ...
                              <div key={idx} style={{ display: "flex", alignItems: "center" }}>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (onAction) onAction("EVENT_HASH_NAVIGATE", `${uc.id}:${idx}`);
                                  }}
                                  style={{
          ...
          {selectedStep && selectedUsecase && (
            <div className="ve-card" style={{ position: "sticky", top: "80px", padding: "0", overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "calc(100vh - 100px)" }}>
              <div style={{ padding: "16px", background: "var(--surface-elevated)", borderBottom: "1px solid var(--border)" }}>
                 ...
                 <h2 style={{ fontSize: "1.1rem", fontWeight: 700, color: "var(--text)", marginBottom: "4px", display: "flex", alignItems: "center", gap: "6px", margin: 0 }}>
              ...
              <div style={{ padding: "16px", overflowY: "auto", flex: 1 }}>
                <div style={{ marginBottom: "20px" }}>
                  <h4 style={{ fontSize: "0.8rem", color: "var(--text)", fontWeight: 600, marginBottom: "8px", display: "flex", alignItems: "center", gap: "6px", margin: 0 }}>
                ...
                <div style={{ marginTop: "24px", paddingTop: "16px", borderTop: "1px dashed var(--border)", textAlign: "center" }}>
                  <button
                    onClick={() => onAction && onAction("EVENT_HASH_NAVIGATE", selectedStep.screen)}
                    className="btn-primary"
                    style={{ width: "100%", fontSize: "0.85rem", padding: "10px", cursor: "pointer" }}
                  >
      ...
      {state === "error" && <ErrorBanner title="Lỗi tải storyboard" message="Storyboard data corrupted." onRetry={() => onAction && onAction("EVENT_REFRESH")} fullpage />}
