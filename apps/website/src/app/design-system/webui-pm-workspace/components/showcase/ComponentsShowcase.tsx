...
export function ComponentsShowcase({
  ...
  const sectionTitle = (sec: typeof componentSections[0]) => (
    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
      <h3 style={{ color: "var(--text)", fontSize: "0.875rem", fontWeight: 600, margin: 0 }}>{sec.label}</h3>
  ...
  return (
    <div aria-label="Components Screen" className="space-y-8">
      {state === "default" && (
  ...
  Agent:  Claude-01`}</pre>
          ...
          <section id="cards">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px" }}>
              <div className="ve-card" style={{ padding: "16px" }}>
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "8px", margin: 0 }}>Base Card</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Default card with hover effect</p>
              </div>
              <div className="ve-card ve-card--hero" style={{ padding: "16px" }}>
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "8px", margin: 0 }}>Hero Card</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Elevated with glow border</p>
              </div>
              <div className="ve-card ve-card--pillar accent-cyan" style={{ padding: "16px" }}>
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "8px", margin: 0 }}>Pillar Cyan</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Left accent border</p>
              </div>
              <div className="ve-card ve-card--pillar accent-rose" style={{ padding: "16px" }}>
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "8px", margin: 0 }}>Pillar Rose</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Critical accent</p>
              </div>
            </div>
          </section>
          ...
          <section id="promptcard">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "16px" }}>
              <div className="prompt-card">
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "4px", margin: 0 }}>🔍 Code Search</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Tìm implementation của MVCC trong FrankenSQLite</p>
              </div>
              <div className="prompt-card">
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "4px", margin: 0 }}>📋 Sprint Summary</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Tổng hợp các task hoàn thành trong sprint hiện tại</p>
              </div>
              <div className="prompt-card disabled" aria-disabled="true">
                <h4 style={{ color: "var(--text)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: "4px", margin: 0 }}>🔒 Admin Only</h4>
                <p style={{ color: "var(--text-dim)", fontSize: "0.75rem", margin: 0 }}>Cần quyền admin để sử dụng</p>
              </div>
            </div>
          </section>
          ...
          <section id="labels">
            ...
            <div style={{ display: "grid", gap: "12px" }}>
              <span className="section-label accent-cyan" style={{ width: "fit-content" }}>Architecture</span>
              <span className="section-label accent-teal" style={{ width: "fit-content" }}>Implementation</span>
              <span className="section-label accent-amber" style={{ width: "fit-content" }}>Review Required</span>
              <span className="section-label accent-rose" style={{ width: "fit-content" }}>Critical Path</span>
            </div>
          </section>
        ...
        <div style={{ display: "grid", gap: "24px" }}>
          {[1, 2, 3].map(i => (
            <div key={i}>
              ...
              <div style={{ marginTop: "8px" }}><Skeleton variant="card" /></div>
            </div>
          ))}
        </div>
      )}
      ...
      {state === "empty" && (
        <EmptyState
          icon="🧩"
          title="Không có components"
          desc="Registry trống. Chạy seed script để tạo components mẫu."
        />
      )}

      {state === "error" && (
        <ErrorBanner
          title="Lỗi tải components"
          message="Failed to read registry.json — invalid JSON format"
      ...
      {state === "offline" && (
        <div className="offline-banner">
          <span className="offline-banner__icon">📡</span>Đang offline — một số components có thể không render đúng
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <div className="forbidden-gate__title">Không có quyền</div>
          <div className="forbidden-gate__desc">Design System chỉ dành cho Developers.</div>
