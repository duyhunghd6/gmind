...
export function ApprovalShowcase({
  ...
  const toggleApproval = (id: string, newState: "pending" | "approved" | "rejected") => {
    ...
    if (onAction) onAction("EVENT_APPROVAL_DECISION", `${id}: ${newState}`);
  };
  ...
  return (
    <div aria-label="Phê duyệt &amp; RTM Screen" className="space-y-6">
      {state === "default" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          {/* Multiple Approval Panels */}
          <div>
            <div id="panels" style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
              <h3 style={{ color: "var(--text)", fontSize: "0.875rem", fontWeight: 600, margin: 0 }}>Panels Phê duyệt ({approvalPanels.length} tasks)</h3>
          ...
          {/* RTM */}
          <div>
            <div id="rtm" style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
          ...
          {/* Heatmap */}
          <div>
            <div id="heatmap" style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
      ...
      {state === "empty" && (
        <EmptyState
          ...
          desc="Chưa có task nào cần Level 3 Approval."
        />
      )}

      {state === "error" && (
        <ErrorBanner
          title="Lỗi tải dữ liệu RTM"
          message="SQL query failed: beads_rust process exited"
      ...
      {state === "offline" && (
        <div className="offline-banner">
          <span className="offline-banner__icon">📡</span>Sync mất kết nối — RTM hiển thị dữ liệu cache
      ...
      {state === "forbidden" && (
        <div className="forbidden-gate">
          ...
          <div className="forbidden-gate__title">Cần quyền Reviewer</div>
          <div className="forbidden-gate__desc">Approval Panel chỉ dành cho QA Reviewer và PMO.</div>
