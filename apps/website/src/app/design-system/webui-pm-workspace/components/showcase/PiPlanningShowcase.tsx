...
export function PiPlanningShowcase({ state = "default", onAction }: { state?: string; onAction?: (e: string, target?: string) => void }) {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (onAction) onAction("EVENT_PI_PLAN_SAVE", `moved ${result.draggableId}`);
  };
  ...
  return (
    <div aria-label="PI Planning Sandbox Screen">
             ...
             <div className="card" style={{ padding: "20px", marginBottom: "24px", background: "var(--surface)" }}>
                ...
                <p style={{ fontSize: "0.8125rem", color: "var(--text-dim)", marginBottom: "16px" }}>
                 ...
                 <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "center" }}>
                    {[1, 2, 3, 4, 5].map(v => (
                      <button key={v} onClick={() => onAction && onAction("EVENT_CONFIDENCE_VOTE", `confidence: ${v}`)} className="btn-secondary" style={{ width: "40px", height: "40px", padding: 0, borderRadius: "50%", fontWeight: "bold" }}>
                        {v}
                      </button>
                    ))}
                 </div>
      ...
      {state === "error" && <ErrorBanner title="Lỗi tải ROAM Board" message="Lỗi kết nối CSDL phân tán." onRetry={() => onAction && onAction("EVENT_REFRESH")} fullpage />}
      {state === "offline" && <div className="offline-banner"><span className="offline-banner__icon">📡</span>CSDL offline — đang hoạt động với local memory cache<span className="offline-banner__timer">3s</span></div>}
      {state === "forbidden" && <div className="forbidden-gate"><div className="forbidden-gate__title">Cần quyền PM/RTE</div><div className="forbidden-gate__desc">Strategic Sandbox yêu cầu quyền Project Manager hoặc Release Train Engineer.</div><div className="forbidden-gate__code">HTTP 403</div></div>}
    </div>
  );
}
