...
export const diagram: DiagramEntry = {
  ...
  quiz: {
    ...
    options: [
      "A. Không nghiêm trọng trong thực tế — major APIs (Stripe, AWS, GCP) tuân thủ Semantic Versioning (semver) nghiêm ngặt: deprecated endpoints vẫn hoạt động ít nhất 2 năm sau deprecation notice, đủ thời gian cho LLM training cycle cập nhật",
      ...
      "C. Chỉ ảnh hưởng API có breaking changes (major version bumps) — hầu hết APIs sử dụng backward-compatible evolution (thêm fields mới, không xóa fields cũ), nên code dùng API cũ vẫn hoạt động bình thường dù không phải version mới nhất",
      "D. LLM có khả năng phát hiện deprecated patterns nhờ training data chứa deprecation warnings — khi generate code dùng old API, LLM tự thêm comment '// TODO: migrate to v3' nhưng vẫn dùng working v2 code vì tương thích ngược"
    ],
    correctIndex: 1,
