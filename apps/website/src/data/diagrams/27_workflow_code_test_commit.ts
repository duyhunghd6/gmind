...
export const diagram: DiagramEntry = {
  ...
  quiz: {
    ...
    options: [
      "Thứ tự Code → Test → Commit được enforce bởi git hooks (pre-commit chạy tests, pre-push chạy lint) — Workflow chỉ mirror quy trình mà git hooks đã enforce ở infrastructure level. Nếu disable git hooks thì Workflow cũng mất tác dụng",
      "Ánh xạ kiến trúc CI/CD (Shift-Left Quality Gate): Workflow bắt buộc bước Code Tests Passed để đảm bảo regression coverage, và Human-in-the-loop Approval chống break changes. Kỷ luật này triệt để ngăn chặn unverified patches merge vào Master.",
      "Workflow là living documentation: nó ghi lại best practices nhưng KHÔNG có cơ chế enforcement thực sự — LLM có thể skip bất kỳ step nào nếu developer yêu cầu ('commit ngay, bỏ qua test'). Enforcement thực nằm ở CI/CD pipeline",
      "Thứ tự này tối ưu token usage: chạy test TRƯỚC commit giúp LLM có test results trong context window → generate commit message chính xác hơn (bao gồm test coverage % và pass/fail ratio). Nếu commit trước test thì commit message thiếu verification data"
    ],
    correctIndex: 1,
